...
def main(
    ...
    # ウィンドウ並列化メモ
    # cw,pwを個別に設定
    # winodowインスタンスをリストで管理し，以下のパラメータを保持する
    # 最小ウィンドウサイズ，過去現在ウィンドウ比率，並列ウィンドウ数，並列ウィンドウ比率

    # ウィンドウサイズ自動適応化メモ
    # 過去ウィンドウサイズを適応的に決定する手法
    ...
    if online_mode:
        ...
        for d_file in sorted(os.listdir(t.d_dir_path)):
            ...
            for row in reader:
                ...
                w.cum_statics+=DD.call(method_code, w.ex_cw(), w.ex_pw())
                print(f"{t.c_time} cum_statics: {w.cum_statics}")
